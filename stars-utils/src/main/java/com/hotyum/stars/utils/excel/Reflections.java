...
public class Reflections {
	 ...
	 */
	public static Object getFieldValueByMethod(final Object obj, final String fieldName) throws Exception {
	 ...
	 * 用于调用的情况，否则应使用getAccessibleMethod()函数获得Method后反复调 . 同时匹配方法 +参数类型
	 */
	 ...
	 * 用于调用的情况，否则应使用getAccessibleMethodByName()函数获得Method后反复调�?.
	...
	/**
	 * 循环向上转型, 获取对象的DeclaredMethod,并强制设置为可访 . 如向上转型到Object仍无法找 , 返回null. 匹配函数
	 * +参数类型
	 * 
	 * 用于方法要被多次调用的情 . 先使用本函数先取得Method,然后调用Method.invoke(Object obj, Object...
	 * args)
	 */
	 ...
	 * 只匹配函数名�?
	 * 
	 * 用于方法�?要被多次调用的情�?. 先使用本函数先取得Method,然后调用Method.invoke(Object obj, Object...
	 * args)
	 */
	...
	/**
	 * 通过反射, 获得Class定义中声明的泛型参数的类�?, 注意泛型必须定义在父类处 如无法找�?, 返回Object.class. eg.
	...
	public static Field[] getAllFields(Class<?> initClass) {
		...
		while (!initClass.getSuperclass().equals(Object.class)) {
			...
			Field[] t_fields = initClass.getDeclaredFields();
			fields = (Field[]) ArrayUtils.addAll(fields, t_fields);
		}
		return fields;
	}
}
